...
function handleInitialMeetingData(data) {
  if (data.questions && data.access_code) {
    ...
    console.log(
      `Loaded ${totalMeetingQuestions} questions for meeting ${accessCode}`
    );
...
function updateQuestionDisplay() {
  ...
  const questionText =
    meetingQuestions[currentQuestionIndex] || `Question ${questionNum}`;
...
function updateSubmissionTracker() {
  ...
  if (submissionElement) {
    ...
    const percentage =
      connectedCount > 0 ? (totalSubmissions / connectedCount) * 100 : 0;
        ...
        (p) =>
          `<p>
                ...
                <span class="connection-status ${
                  p.status === "Disconnected" ? "disconnected" : "connected"
                }">${p.status || "Connected"}</span>
            </p>`
      )
      .join("");
  }
